...
test_that("Loop the Loop", {
  ...
  snapshot_amAlleleFreq <- function(ds, multilocusMap=NULL) {
    ...
    expect_snapshot_value(af, style = "deparse", variant = amvariant)

    if(class(af) == "amAlleleFreq") {
      expect_snapshot(print.amAlleleFreq(af), variant = amvariant)
    } else {
      expect_snapshot(cat(af), variant = amvariant)
    }
